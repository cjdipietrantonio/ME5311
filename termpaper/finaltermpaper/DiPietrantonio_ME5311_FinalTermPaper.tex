...
The paper begins by establishing the financial definitions and background necessary to understand the content of later sections, then derives the BSM equation following the approach outlined by Hull~\cite{Hull2022}, Black and Scholes~\cite{BlackScholes1973}, and Merton~\cite{Merton1973}. The transported PDF method is then introduced following the work of Pope~\cite{Pope2000}, and its connection with the BSM equation is established through comparison of the underlying SDEs and variable mapping. The paper continues to discuss the solution methods for the BSM and the transported PDF equation, which include grid-based (Eulerian) and particle-based (Lagrangian) approaches. A Python program that implements analytical, Eulerian (finite-difference), and Lagrangian (Monte Carlo) solution methods for pricing a European call option is used to explore these methods in greater detail. The benefits and pitfalls of the different solution approaches are also explored. The paper concludes by discussing the real-world applications of this cross-disciplinary connection and how advancements in either discipline can potentially benefit the other.
...
\begin{equation}
    ...
    \label{eq:gbm}
\end{equation}
where $\mu$ is the stock's expected rate of return (under risk-neutral valuation, $\mu = r$, the risk-free rate), $\sigma$ is the volatility of the stock price, and $dW$ is a Wiener process (analogous to $dz$)~\cite{Hull2022, Shreve2004}. This model is commonly referred to as geometric Brownian motion. The first term, $\mu S\, dt$, represents the deterministic drift of the stock's price, and the second term, $\sigma S\, dW$, represents random fluctuations. For the remainder of this derivation and paper, $\mu$ is replaced with $r$ under risk-neutral valuation~\cite{Hull2022}. 

\subsection{Applying It\^{o}'s Lemma to the Option Value}
Suppose $V(S, t)$ is the value of a European call option dependent on the price of stock $S$. Given that the price of $S$ is modeled using the It\^{o} process above, It\^{o}'s lemma (Equation~\ref{eq:ito_lemma}) can be applied, and the result can be rearranged using Equation~\ref{eq:gbm} to obtain~\cite{Hull2022}:
\begin{equation}
...
Both the Black-Scholes-Merton and Lagrangian PDF Transport equations are derived from stochastic differential equations of the same drift plus diffusion form. Comparing the geometric Brownian motion SDE (Equation~\ref{eq:gbm}) with the generalized Langevin SDE (Equation~\ref{eq:langevin}) reveals that both equations have the same mathematical structure: deterministic drift term multiplied by $dt$, plus a diffusion term (random fluctuations) driven by a Wiener process, $dW$. Furthermore, both PDEs can be derived from their respective SDEs using the same mathematical tools, Stochastic Calculus and It\^{o}'s lemma.
...
where $\theta(t) = \ln(S_0) + (r - \sigma^2/2)t$ and $\omega(t) = \sigma \sqrt{t}$ are the mean and standard deviation of $\ln(S)$ respectively, and $t$ is the current time step. Second, it allows for an exact step formula for the SDE to be obtained~\cite{Hull2022}.
\begin{equation}
    S(t + \Delta t) = S(t) \cdot \exp\left[\left(r - \frac{1}{2}\sigma^2\right)\Delta t + \sigma\epsilon\sqrt{\Delta t}\right]
    \label{eq:exact_step}
\end{equation}

where $\epsilon$ is a random sample from a normal distribution with a mean of 0 and a standard deviation of 1.0. For the Langevin SDE, because the coefficients $\mathbf{a}$ and $b$ depend on the particle's velocity and position at a given time, the Langevin SDE must be discretized directly:
\begin{equation}
...
After substituting the stencils above into the PDE and writing $S_j = j \Delta S$, a tridiagonal system of equations is obtained for each time step. This tridiagonal system has the general form of:
\begin{equation}
...
The Monte Carlo simulation was implemented using the exact step formula for the Geometric Brownian motion SDE (Equation~\ref{eq:exact_step}). 200,000 random paths were simulated to the final time $T$ for 80 stock prices between \$50 and \$200. The value of the option at $t = 0$ can then be obtained using the risk-neutral valuation formula, by discounting the expected (average) payoff at maturity back to the current time step at the risk-free rate~\cite{Hull2022}.
\begin{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{../BSM_Solver/figures/figure01_option_value_curve.png}
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \vspace{0pt}
        \centering
        \includegraphics[width=\textwidth, height=7cm, keepaspectratio]{../BSM_Solver/figures/figure02_GMB_sample_paths.png}
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \vspace{0pt}
        \centering
        \includegraphics[width=\textwidth, height=7cm, keepaspectratio]{../BSM_Solver/figures/figure04_2D_pdf_slices.png}
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \vspace{0pt}
        \centering
        \includegraphics[width=\textwidth, height=7cm, keepaspectratio]{../BSM_Solver/figures/figure06_pdf_heatmap.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{../BSM_Solver/figures/figure05_MC_convergence.png}
