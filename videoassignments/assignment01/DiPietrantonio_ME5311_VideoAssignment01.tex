...
\noindent Now, we have already defined kinetic energy, $k = \frac{1}{2}\left(u^2 + v^2 + w^2\right)$. If we take the material derivative of k, we are left with:
...
\begin{equation}
\frac{D}{Dt}\left(\frac{1}{2}w^2\right) = w\frac{Dw}{Dt}
\end{equation}

\noindent Therefore, we have:

\begin{equation}
\frac{Dk}{Dt} = u\frac{Du}{Dt} + v\frac{Dv}{Dt} + w\frac{Dw}{Dt}
\end{equation}

\noindent And we can re-write the left hand side of our equation as:

\begin{equation}
\rho \left(u\frac{Du}{Dt} + v\frac{Dv}{Dt} + w\frac{Dw}{Dt}\right) = \rho\frac{Dk}{Dt}
\end{equation}

\noindent Now, we can turn our attention to the right hand side of our equation. Combining all of the pressure terms yields:
...
\noindent We can now combine all of the viscous stress terms. Recalling that from x-momentum we have:
...
\begin{equation}
uS_{Mx} + vS_{My} + wS_{Mz} = \vec{u} \cdot \vec{S}_M
\end{equation}

\noindent Finally, we can combine all aforementioned terms to get the governing equation for the kinetic energy:
...
\noindent As mentioned in ``An Introduction to Computational Fluid Dynamics: The Finite Volume Method'' by H.K. Versteeg and W. Malalasekera, deriving the governing equation for the kinetic energy allows us to separate the kinetic/mechanical contributions (effects of pressure forces, viscous stresses, and related source terms) to the total energy of the fluid from the contributions of internal energy, heat transfer, viscous dissipation, and other sources. Doing so also allows us to obtain expressions for quantities such as internal energy, $i$ or temperature, $T$.
