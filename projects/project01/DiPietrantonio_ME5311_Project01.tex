...
We can now program a numerical solver (using LU decomposition as the linear system solver) to solve the discretized governing equations above and compare the results with the analytical solution derived in the previous section. 
...
We include both error at the selected x-locations (for direct comparison with the plotted profiles) and the error at every x-step (to demonstrate stability of the Crank Nicholson scheme).
