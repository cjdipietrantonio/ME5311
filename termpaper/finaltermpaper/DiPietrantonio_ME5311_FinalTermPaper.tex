...
The use of Stochastic Calculus in the derivation of Equation~\ref{eq:bsm} is crucial to the argument of this paper. Therefore, this section will walk through such derivation as done so by Hull~\cite{Hull2022}, Black and Scholes~\cite{BlackScholes1973}, and Merton~\cite{Merton1973}.
...
The Eulerian approach solves the partial differential equation directly on a fixed grid. While Eulerian methods are efficient in low dimensions, and provide a deterministic solution, they scale poorly to high dimensions. For example, in order to simulate the price of a portfolio of two stocks, if each stocks price axis was discritized using 500 points, the solver would require 250,000 grid locations to cover all possible combinations. The same cocnept can be applied to the transported PDF method for high-dimensional PDFs. This is the exact reason, when refering to the velocity-frequency joint PDF, that Pope states is not feasible, computationally, to represent the joint PDF accurately through a discretization of the seven-dimensional $V$-$\theta$-$x$ space, as is required in finite-difference, finite-volume, and finite-element methods~\cite{Pope2000}. Therefore, the transported PDF equations are modeled using langragian methods.

\subsection{Lagrangian Methods}
The Lagrangian approach simulates random particle trajectories using the respective underlying stochastic differential equation, then uses statistics to obtain the desired information from the resulting data. The Lagrangian approach most typically used to solve the Black-Scholes-Merton equation is the Monte Carlo method. For thsi, many random stock price paths are simulated using the Geometric Brownian Motion SDE. The payoff is computed for each path, and the option value is estimated as the average payoff discounted back to the current time. For the transported PDF method, the Lagrangian particle method is used. In this case, the velocities of many fluid particles in a turbulent flow are simulated using the Langevin SDE, and the resuting PDF and its transport is constructed from the particle statistics~\cite{Pope2000}.

The primary advantage of Lagrangian methods is that they scale well with high dimensions. Adding additional state variables (e.g., more stocks or velocity compnents) just means more longer vectors are simulated. In this case, the computational cost grows linearly with added dimensions, whereas with Eulerian methods it grows exponentially. Furthermore, since each path is independent from one another, Lagrangian methods are more compatible with GPUs and High Performance Clusters, which can parallelize computations across many processing units. The primary limitation of Lagrangian models is statistical noise. The standard error of a Motne Carlo analysis is proportional to $1/\sqrt{N}$, where $N$ is the number of simulated paths~\cite{Hull2022}. Therefore, to cut the error in half, four times as many paths are needed, and to reduce the error by a factor of ten, 100 times as many paths are required

\subsection{Constant vs. Variable Coefficients}
As mentioned previously, the coefficients of the Geometric Brownian Motion and Langevin SDEs have a significant impact on the available solution methods. The constant coefficients of the Geometric Brownian Motion SDE allow for It\^{o}'s lemma to be aplied to $\ln(S)$, which shows that $\ln(S)$ follows a generalized Wiener process, and further that $\ln(S_T)$ is normally distributed. This is significant for two reasons. First, because $\ln(S_t)$ is normally distributed, then $S_t$ must be lognormally distributed. Therefore, the standard equation for the probability density function of a lognormal variable can be used~\cite{Montgomery2011}.
\begin{equation}
...
Where $\epsilon$ is a random sample from a normal distribution with a mean of 0 and a standard deviation of 1.0. For the Langevin SDE, because the coefficients $\mathbf{a}$ and $b$ depend on the particle's velocity and position at a given time, the Langevin SDE must be discritized directly:
\begin{equation}
...
The BSM PDE (Equation~\ref{eq:bsm}) was discritized using an implicit finite diference scheme. The stock price axis was discretized into a uniform grid with 500 intervals from $S = \$0$ to $S_{\max} = \$500$, and time was discritized into 5000 steps marching backwards from $t = T$ to $t = 0$. Second order central difference stencils were used for the first and second spatial derivatves, and a forward difference was used for the temporal term. 

After substituting the stencils above into the PDE, and by writing $S_j$ as $\Delta S$, a trigiagonal system of equations is obtained for each time step. This tridiagonal system has the general form of:
\begin{equation}
...
To visualize the stochastic nature of the Monte Carlo simulation, Figure~\ref{fig:sample_paths} shows 30 individual stock price paths sampled from 100 simulated paths, all starting at $S_0 = \$100$. The difussion of the paths over time demonstrates the effect of volatility. The bold blue line represents the mean of the 100 simulated paths, and the black dashed line represents the theoretircal expected stock price, $E(S_t) = S_0 e^{rt}$~\cite{Hull2022}. As the number of simulated paths increases (and with this the accuracy of the Monte Carlo simulation), the mean converges to this theoretical value.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../BSM_Solver/figures/figure02_GMB_sample_paths.png}
...
The data from the Monte Carlo simulation was then used to construct the probability density function of the stock price at each time step. This is the step that reinforces the mapping between Monte Carlo simulations and Lagrangian particle methods. For a turbulent flow, particle paths would be simualted using the Langevin SDE, their velocities (conditioned on position and time) would be recorded at each time step, and the PDF surface would be constructed from the statistics. 

Figure~\ref{fig:pdf_3d} shows the 3D PDF evolution surface for s tock with initial price $S_0 = \$100$. The surface was constructed entirely from Monte Caro particle data, as 200,000 simulated paths were used to create a histogram at selected time steps. The PDF starts as a sharp spike near the initial stock price, then shifts and spreads over time due to drift and diffusion.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../BSM_Solver/figures/figure03_3d_pdf_evolution.png}
    \caption{PDF evolution from Monte Carlo particle data}
    \label{fig:pdf_3d}
\end{figure}

Figure~\ref{fig:pdf_slices} provides a clearer view of the drift and diffusion of the PDF over time. It also further demonstrates the accuracy of the Monte Carlo simulation by comparing its results to the analytical expression for the probability density function of a lognormal variable (Equation~\ref{eq:analytical_logpdf}'). The shaded bars are the PDFs constructed from the Monte Carlo data plotted as histograms, and the solid curves are the analytical lognormal PDFs. Over time, the volatility spreads the distribution of possible stock prices (diffusion), and the drift shifts it to the right (advection). For a turbulent flow, drift can be thought of as where the average velocity trends, and diffusion as the effect of turbulence randomizing the velocity around the mean. 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../BSM_Solver/figures/figure04_2D_pdf_slices.png}
    \caption{PDF Evolution: Monte Carlo histograms vs. log-normal PDF.}
    \label{fig:pdf_slices}
\end{figure}

Figure~\ref{fig:pdf_heatmap} shows a top-down heatmap view of the PDF evolution, with the expected stock price $E(S_t) = S_0 e^{rt}$ (white dashed line) and the $\pm 1 \sigma$ bounds (white dotted lines) overlaid. The drift of the PDF is clearly visible, aligning with the expected stock price, which can also be thought as the deterministic dorwth of the stock in the absence of volatility.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../BSM_Solver/figures/figure06_pdf_heatmap.png}
...
%         \vspace{0pt}
%         \centering
%         \includegraphics[width=\textwidth]{../BSM_Solver/figures/figure04_2D_pdf_slices.png}
%         \caption{PDF Evolution: Monte Carlo histograms vs. log-normal PDF.}
%         \label{fig:pdf_slices}
%     \end{minipage}
%     \hspace{8pt}
...
%         \vspace{0pt}
%         \centering
%         \includegraphics[width=\textwidth]{../BSM_Solver/figures/figure06_pdf_heatmap.png}
%         \caption{PDF evolution heatmap with drift path and $\pm 1\sigma$ bounds.}
%         \label{fig:pdf_heatmap}
%     \end{minipage}
% \end{figure}
...
Finally, Figure~\ref{fig:mc_convergence} demonstrates the convergance behavior of the Monte Carlo method. The standard error of the estimate produced by the simulation is expressed as $\omega/\sqrt(N)$~\cite{Hull2022}. Where $\omega$ is the standard deviation of the discounted payoffs, and $N$ is the number of simulated paths. With 100 paths, the estimate is noisy because each path has a greater weight. However, by 1,000,000 paths, the estimate approaches the exact value of obtained using the Black-Scholes formula. As mentioned in Section~\ref{sec:solution_methods}, this slow convergance rate is a major limitation of Lagrancien approches.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../BSM_Solver/figures/figure05_MC_convergence.png}
...
% ----------------------------------------------------------------------
\section{Conclusions}
The contents of this paper and the results of the computational experimants have demonstrated the mathematical connection between the Black-Scholes-Merton equation for pricing financial derivatives, and the transported probability density function method used in computational fluid dynamics. From the material presented in this paper, several conclusions can be drawn.
...
Second, both Eulerian (finite difference) and Lagrangian (Monte Carlo) methods produce results consistant with not ony eachother, but also analytical solutions. This was validated by the agreement between the numerical results obtained through the experimental Eulerian and Lagrangian solvers andthe exact values obtained using the analytical Black-Scholes pricing formula. 

Third, Monte Carlo particle simulations can reproduce results consistent with analytical probability desnity functions, as demonstrated in Section~\ref{subsec:pdf_transport} and Figure~\ref{fig:pdf_slices}. This is an extremely important conclusion in the case of the transported PDF method, where no analytical probability density function exits.


Fourth, in the case of the Black-Scholes-Merton equation, an analytical solution exists for the Geometric Brownian Motion SDE of a stock price which implies that a stocks price at a future time (given its initial price) is lognormally distributed. As a result, the analytical expression for the probabillity densitiy function of a lognormal random variable can be used to obtain the PDF. Therefore, the BSM equation can be used to validate Lagrangian particle methods, because the PDFs generated from the particle methods can be compared with those obtained analytically, as demonstrated with the reslts of the Monte Carlo experiment discussed in this paper.

Finally, advancements in fincncial modeling and computational fluid dynamics are mutually beneficial. The shared mathematical framework illustrated in this paper means that numerical methods, variance reduction techniques, and other solution strategies developed in thefields of quantatative finance and computational fluid dynamics can possibly be utlilized or even directly applied to problems in the other.
