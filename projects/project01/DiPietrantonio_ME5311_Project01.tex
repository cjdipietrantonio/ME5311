...
The numerical solution of the parabolic equation found using the Crank-Nicholson and central difference schemes show excellent agreement with the analytical solution derived in Section 2. Figure~\ref{appendix-code} compares nunmerical and analytical profiles of $u$ at several $x$-locations, and for all values of $x$ the two curves lie nearly on top of one another. This demonstrates that the discritization accurately captures both the transient development in the $x$-direction and the parabolic shape in the $y$-direction. Tha analytical solution contains an exponentially decaying transient term proportional to $e^{-2 x \pi^2}$, the solution approaches the steady state profile
\begin{equation}
u(y) = \frac{y}{2} (1-y)
\end{equation}
as $x$ increases. At roughly $x = 0.25$, the transient component of the solution has decayed to about 1\% of its initial magnitude, and the solution is effectivly steady. This behavior is confirmed in the residual plot (Figure~\ref{fig:residual_vs_x}), as the L2 Residual inititally decreases rapidly initially, then remains inchanged at a value of roughly zero, indicating no change between successive steps in $x$. Finaly, the parabolic shape we see the solution approach as $x$ increases makes sense intuitively because each cell contributes a constant source term of $2 \Delta x$, while the boundary conditions enforce $u=0$ at $y=0$ and $y=1$.Therefore, the cells further from the boundaries will have larger values of $u$, as the flux with respect to $u$ has a greater distance in $y$ to diffuse through before reaching the boundaries.

\subsection{Accuracy and stability}

Analytical profiles at different x-locations are nearly indistinguishable from their respective numerical curves. Figure~\ref{fig:error_vs_x_all} includes the eror at every x-step to demonstrate the accuracy and stability of the simulation/numerical scheme. From Figure~\ref{fig:error_vs_x_all}, one can observe that the error does not increase past 0.008 thus demonstrating the accuracy of the numerical solution across the entire domain. Apart from slight fluctuations in the error across teh first few time steps (LIKELY DUE TO?), one can also observe that there are no singificant oscilations, or unphysical values in both Figures \ref{fig:profiles_comparison} and \ref{fig:error_vs_x_all}, thus confirming the stability of the numerical scheme. This is to be expected, as the Crank-Nicholson scheme is unconditionally stable. 
